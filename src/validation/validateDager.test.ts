...
describe('validateDager', () => {
  ...
  it('should verify that dager is numeric when required and dager is 0', () => {
    ...
    expect(validateDager('notanumber', 1000, true, undefined, undefined)?.key).toBe('VALIDATE_DAGER_DIGITS_ONLY');
  });
  ...
  it('should return an error when the dager string has no content', () => {
    ...
    expect(validateDager('', 1000, true, undefined, undefined)?.key).toBe('VALIDATE_DAGER_MISSING');
  });

  it('should return an error when dager is undefined', () => {
    expect(validateDager(undefined, 1000, true, undefined, undefined)).not.toBeUndefined();
    expect(validateDager(undefined, 1000, true, undefined, undefined)?.key).toBe('VALIDATE_DAGER_MISSING');
  });
  ...
  it('should return an error when dager is more than max', () => {
    ...
    expect(validateDager('2000', 1000, true, undefined, undefined)?.key).toBe('VALIDATE_DAGER_TOO_HIGH');
  });
  ...
  it('should return an error when there are more dager than available days', () => {
    ...
    expect(validateDager('20', 1000, true, fra, til)?.key).toBe('VALIDATE_DAGER_INVALID');
  });
});
