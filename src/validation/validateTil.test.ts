...
describe('validateTil', () => {
  ...
  it('should not show errors when valid', () => {
    expect(
      validateTil(
        parseDato('01.02.2021'),
        parseDato('03.04.2021'),
        minDate,
        false
      )
    ).toBeUndefined();
  ...
  it('should not show errors when fra is undefined', () => {
    expect(
      validateTil(undefined, parseDato('03.04.2021'), minDate, true)
    ).toBeUndefined();
  });

  it('should not show errors when til is undefined and not required', () => {
    expect(
      validateTil(parseDato('03.04.2021'), undefined, minDate, false)
    ).toBeUndefined();
  });

  it('should show error when til is undefined', () => {
    expect(
      validateTil(parseDato('03.04.2021'), undefined, minDate, true)
    ).not.toBeUndefined();
  });
});
